...
namespace PhoneSkillTest.Flow.LuisTestUtils
{
    public class PhoneSkillTestUtil
    {
        ...
        public static MockLuisRecognizer CreateRecognizer()
        {
            ...
            {
                AddUtterance(OutgoingCallDialogUtterances.OutgoingCallNoEntities, PhoneLU.Intent.OutgoingCall);

                AddUtterance(OutgoingCallDialogUtterances.OutgoingCallContactName, PhoneLU.Intent.OutgoingCall, new List<InstanceData>()
                {
                    CreateEntity("contactName", "bob", 5),
                });

                AddUtterance(OutgoingCallDialogUtterances.OutgoingCallPhoneNumber, PhoneLU.Intent.OutgoingCall, new List<InstanceData>()
                {
                    CreateEntity("phoneNumber", "0118 999 88199 9119 725 3", 5),
                });

                AddUtterance(OutgoingCallDialogUtterances.RecipientContactName, PhoneLU.Intent.OutgoingCall, new List<InstanceData>()
                {
                    CreateEntity("contactName", "bob", 0),
                });

                AddUtterance(OutgoingCallDialogUtterances.RecipientPhoneNumber, PhoneLU.Intent.OutgoingCall, new List<InstanceData>()
                {
        ...
        private static void AddUtterance(string userInput, PhoneLU.Intent intent, IList<InstanceData> entities = null)
        {
            _utterances[userInput] = CreateIntent(userInput, intent, entities);
        }

        private static PhoneLU CreateIntent(string userInput, PhoneLU.Intent intent, IList<InstanceData> entities = null)
        {
